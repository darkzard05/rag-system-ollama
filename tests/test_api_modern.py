    ...
    with patch.object(api_server, "SessionManager") as mock_sm:
        ...
        async def async_gen(*args, **kwargs):
            events = [
                ...
                {"event": "on_chain_end", "name": "retrieve", "data": {"output": {"documents": []}}}
            ]
            for event in events:
                yield event
...
@pytest.mark.asyncio
async def test_stream_query_success(async_client, mock_rag_resources, mock_session_manager):
    """스트리밍 질의 성공 시나리오 테스트"""
    payload = {"query": "Hello?", "use_cache": True}
    
    async with async_client.stream("POST", "/api/v1/stream_query", json=payload) as response:
        assert response.status_code == 200
