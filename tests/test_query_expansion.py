...
from src.common.config import QUERY_EXPANSION_PROMPT
...
class TestQueryExpansion:
    ...
    async def test_graph_query_expansion_logic(self):
        ...
        async def mock_llm_func(input_val):
            ...
            if "3 search queries" in prompt_str or "similar to the user's question" in prompt_str:
        ...
        mock_retriever.ainvoke = AsyncMock(return_value=[Document(page_content="test doc")])
        ...
        expansion_calls_simple = [
            call for call in llm_call_tracker.call_args_list
            if "3 search queries" in str(call.args[0])
        ]
        assert len(expansion_calls_simple) == 0, "단순 질문인데 쿼리 확장이 수행되었습니다."

        # Retriever는 원본 질문으로 호출되어야 함
        retriever_calls_simple = [str(call.args[0]) for call in mock_retriever.ainvoke.call_args_list]    
        assert simple_input in retriever_calls_simple, "원본 질문으로 검색하지 않았습니다."

        # --- Case 2: 복잡한 질문 ---
        complex_input = "이 시스템의 아키텍처와 성능 최적화 전략에 대해 자세히 설명해주고, 특히 캐싱 전략 이 어떻게 구성되어 있는지 알려줘." * 2
        ...
        assert len(expansion_calls_complex) > 0, "복잡한 질문인데 쿼리 확장이 수행되지 않았습니다."
        
        # Retriever 호출 확인
        retriever_calls_complex = [str(call.args[0]) for call in mock_retriever.ainvoke.call_args_list]
