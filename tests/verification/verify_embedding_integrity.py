...
def verify_integrity():
    ...
    # 검증 1: 개수 및 차원 확인
    assert len(split_docs) == len(reused_vectors), (
        "청크와 벡터의 개수가 일치하지 않습니다!"
    )
    assert reused_vectors[0].shape[0] == 384, (
        f"벡터 차원이 올바르지 않습니다: {reused_vectors[0].shape[0]}"
    )
    ...
    # 인덱스 A: 재사용된 벡터 사용
    text_embeddings = zip(
        [d.page_content for d in split_docs], reused_vectors, strict=False
    )
    vector_store_reused = FAISS.from_embeddings(
        text_embeddings, embedder, metadatas=[d.metadata for d in split_docs]
    )
    ...
    print(f"\n[질의]: {query}")
    print(
        f"[재사용 방식 결과]: {doc_reused.page_content[:50]}... (점수: {score_reused:.4f})"
    )
    print(
        f"[새로 임베딩 결과]: {doc_fresh.page_content[:50]}... (점수: {score_fresh:.4f})"
    )
    ...
    cos_sim = np.dot(reused_vectors[0], fresh_vector) / (
        np.linalg.norm(reused_vectors[0]) * np.linalg.norm(fresh_vector)
    )
    print(f"✅ 검증 3: 두 방식 간 벡터 유사도: {cos_sim:.4f}")

    if cos_sim > 0.9:
        print(
            "결론: 최적화된 임베딩 재사용 방식은 원본 방식과 90% 이상 유사하며 무결합니다."
        )
    else:
