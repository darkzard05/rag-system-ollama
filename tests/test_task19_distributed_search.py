...
from src.services.distributed.distributed_search import (
    DistributedSearchExecutor, SearchQuery, SearchResult,
    NodeSearchStatus, SearchStatus
)
from src.core.search_aggregator import (
    SearchResultAggregator, AggregationStrategy, DuplicateStrategy,
    AggregationMetrics
)
from src.core.reranker import (
    DistributedReranker, RerankerStrategy, RerankingResult,
    SimilarityCalculator, DiversityCalculator
)
...
class TestSearchAggregation:
    ...
    def test_09_weighted_score(self, executor, aggregator, query_embedding):
        ...
        for result in aggregated:
            ...
            assert result.score_adjustments >= 0 if hasattr(result, 'score_adjustments') else True
...
class TestReranking:
    ...
    def test_13_rerank_by_cross_encoder(self, executor, reranker, query_embedding):
        """교차 인코더 재순위지정"""
        query_id = executor.submit_query("cross encoder test", query_embedding, top_k=20)
        results = executor.execute_distributed_search(query_id)
...
class TestEndToEnd:
    ...
    def test_19_complete_search_pipeline(self, executor, aggregator, reranker, query_embedding):
        """완전한 검색 파이프라인"""
        # 1단계: 분산 검색
    ...
    def test_20_search_with_all_strategies(self, executor, aggregator, reranker, query_embedding):
        """모든 전략 테스트"""
...
class TestIntegrationValidation:
    ...
    def test_21_full_system_integration(self, executor, aggregator, reranker, query_embedding):
        """전체 시스템 통합 검증"""
        # 여러 쿼리 실행
        query_ids = []
        for i in range(3):
