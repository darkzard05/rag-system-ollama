...
class TestIntegrityFixes(unittest.TestCase):
    ...
    def test_session_isolation_deepcopy(self):
        ...
        self.assertNotIn("S1 Start", TSM.get("status_logs"), "S2 should not share status logs with S1")
    ...
    def test_document_pooling_metadata_integrity(self):
        ...
        doc1 = Document(page_content="Common content", metadata={"source": "doc1.pdf", "page": 1})
        doc2 = Document(page_content="Common content", metadata={"source": "doc2.pdf", "page": 5})
        ...
        self.assertEqual(len(doc_pool), 2, "Doc pool should contain 2 unique entries for different metadata")
...
class TestAsyncIntegrityFixes(unittest.IsolatedAsyncioTestCase):
    async def test_streaming_buffer_flush_integrity(self):
        ...
        self.assertEqual(full_text, "".join(tokens), "All tokens should be preserved in final output")
        self.assertEqual(len(dispatched_chunks), 3, "Should have 3 dispatch calls: T1, [T2-T6], [T7]")
        self.assertEqual(dispatched_chunks[-1], "T7", "Last chunk should be flushed in finally block")
