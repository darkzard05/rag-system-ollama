...
def test_citation_rendering_integrity():
    ...
    mock_docs = [
        Document(
            page_content="This is content from page 1.",
            metadata={"page": 1, "source": "test.pdf"},
        ),
        Document(
            page_content="Special characters: < > & \" ' \nNew line here.",
...
async def simulate_ui_throttling():
    ...
    print(
        f"시뮬레이션 시작: 총 {total_chunks} 청크, 생성 간격 {chunk_interval * 1000}ms, UI 주기 {throttling_period * 1000}ms"
    )
...
def test_markdown_edge_cases():
    ...
    print(
        f" - 표 내부 인용구 처리: {'✅ PASS' if has_html and has_table_pipe else '❌ FAIL'}"
    )
