...
def run_benchmark():
    ...
    for i, doc in enumerate(docs_para):
        if doc.metadata["page"] != i + 1:
            ...
            if i > 0 and docs_para[i].metadata["page"] <= docs_para[i-1].metadata["page"]:
                order_correct = False
                break
    ...
    if page_count_match and len(docs_seq) > 0:
        content_match = (docs_seq[0].page_content == docs_para[0].page_content and 
                         docs_seq[-1].page_content == docs_para[-1].page_content)
