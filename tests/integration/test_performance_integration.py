...
class TestPerformanceIntegration(unittest.TestCase):
    ...
    def test_all_operation_types_tracked(self):
        ...
        tracked_types = {
            stat for stat in all_stats if all_stats[stat].total_operations > 0
        }
        assert len(tracked_types) == len(operation_types)
    ...
    def test_performance_report_generation(self):
        ...
        assert total_tokens == 50 + 500 + 1000 + 300

        # 각 오퍼레이션 타입별 통계
        for _op_type_key, op_stats in report["operations"].items():
            assert "total" in op_stats
            assert "successful" in op_stats
            assert "duration" in op_stats
    ...
    def test_response_time_percentiles(self):
        ...
        assert stats.avg_duration_seconds > stats.min_duration_seconds

        # 백분위수 확인 (대략적인 범위)
        assert stats.p50_duration_seconds > stats.min_duration_seconds
        assert stats.p95_duration_seconds >= stats.p50_duration_seconds
        assert stats.p99_duration_seconds >= stats.p95_duration_seconds
        assert stats.p99_duration_seconds <= stats.max_duration_seconds
