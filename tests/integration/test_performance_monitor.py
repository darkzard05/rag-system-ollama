...
class TestPerformanceMonitor(unittest.TestCase):
    ...
    def test_operation_metrics(self):
        ...
        with self.monitor.track_operation(
            OperationType.EMBEDDING_GENERATION, {"model": "e5"}
        ) as op:
            time.sleep(0.05)
            op.tokens = 50
    ...
    def test_operation_type_filtering(self):
        ...
        retrieval_count = self.monitor.get_operation_count(
            OperationType.DOCUMENT_RETRIEVAL
        )
        embedding_count = self.monitor.get_operation_count(
...
class TestMonitoringIntegration(unittest.TestCase):
    ...
    def test_full_rag_simulation(self):
        ...
        # Document retrieval
        with self.monitor.track_operation(
            OperationType.DOCUMENT_RETRIEVAL, {"docs": 10}
        ) as op:
            time.sleep(0.01)
            op.tokens = 500
        ...
        # LLM inference
        with self.monitor.track_operation(
            OperationType.LLM_INFERENCE, {"model": "qwen"}
        ) as op:
            time.sleep(0.05)
            op.tokens = 300
