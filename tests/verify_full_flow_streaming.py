...
async def test_full_rag_flow_streaming():
    ...
    if not os.path.exists(pdf_path): pdf_path = "tests/2201.07520v1.pdf"
    
    print(f"STEP 2: RAG 파이프라인 구축 (캐시 확인 중...)")
    ...
    try:
        ...
        async with aclosing(qa_chain.astream_events({"input": question}, config=config, version="v1")) as event_stream:
            async for event in event_stream:
                ...
                # 답변 생성 토큰 감지
                    ...
                    if hasattr(content, "content"): content = content.content
                    
                    if content:
                        if first_token_time is None:
                            first_token_time = time.time() - start_inference
                            print(f"   ✨ 첫 번째 토큰 수신 완료! (지연 시간: {first_token_time:.2f}초)")
        ...
        print(f"   - 첫 토큰 지연(TTFT): {first_token_time if first_token_time else 'N/A'}")
        print(f"   - 생성된 토큰 수: 약 {token_count}개")
        print(f"   - 평균 생성 속도: {token_count / inference_time:.2f} tokens/sec" if token_count > 0 else "   - 생성 속도 측정 불가")
