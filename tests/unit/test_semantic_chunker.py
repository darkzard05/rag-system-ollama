...
async def test_default_cache_manager_is_memory_only():
    ...
    # 메모리 캐시는 켜져 있어야 하고, 영구 디스크 캐시(L3)도 켜져야 함
...
async def test_chunking_does_not_write_response_cache_to_disk(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("common.config.MODEL_CACHE_DIR", str(model_cache))
    # semantic_chunker 는 MODEL_CACHE_DIR 을 import 시점에 바인딩하므로 해당 모듈 속성도 패치
    monkeypatch.setattr("core.semantic_chunker.MODEL_CACHE_DIR", str(model_cache))
    response_cache = model_cache / "response_cache"
...
async def test_memory_only_cache_still_roundtrips():
    ...
    # set 후 get 히트 (L1 메모리에서 즉시 복구)
    payload = {"vector": [0.1, 0.2, 0.3], "cache_version": "1.0"}
    await cm.set("emb:mock-model:abc", payload, persist_to_disk=True)
    fetched = await cm.get("emb:mock-model:abc")
    assert fetched == payload

    # 영구 디스크 캐시(L3)가 활성화되어 동일 콘텐츠 재수집 시 Ollama 왕복을 건너뛴다
    assert cm.disk_cache is not None
