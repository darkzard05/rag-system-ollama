# ModelManager의 모델 필터링 로직을 테스트하는 유닛 테스트
import pytest
...
def test_get_filtered_models_keywords():
    ...
    # 임베딩으로 분류되어야 하는 모델들
    embed_expected = ["my-embed-model", "bge-large", "nomic-v1", "mxbai-embed", "snowflake-arctic"]
...
def test_get_filtered_models_empty_input():
    """빈 입력에 대한 폴백 테스트"""
...
def test_get_filtered_models_sorting():
    """결과 정렬 테스트"""
    available_models = ["z-model", "a-model", "bge-z", "bge-a"]
    
    result = ModelManager.get_filtered_models(available_models)
    
    assert result["llm"] == sorted(result["llm"])
    assert result["embedding"] == sorted(result["embedding"])
