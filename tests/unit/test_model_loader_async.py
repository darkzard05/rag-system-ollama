...
async def test_clear_vram_async():
    ...
    with mock.patch("core.model_loader.load_llm", return_value=mock.MagicMock()):
        ...
        # ModelManager now delegates to ResourceCoordinator; check its pool
        rc = get_resource_manager()
        assert any("to_be_cleared" in k for k in rc.models._pool.keys())
        await ModelManager.clear_vram()
        assert len(rc.models._pool) == 0
