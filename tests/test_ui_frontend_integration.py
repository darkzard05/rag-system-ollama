...
class FrontendIntegrationSimulator(unittest.TestCase):
    ...
    def simulate_ui_render_cycle(self, user_input=None):
        ...
        with patch('core.thread_safe_session.st.session_state', self.mock_st_state), \
    ...
    def test_rendering_persistence_across_multiple_runs(self):
        ...
        # Run 1 결과 확인
        self.assertEqual(len(self.mock_st_state["messages"]), 2, "첫 사이클 후 메시지는 2개여야 합니다.")
        
        print("\n--- [Run 2] 리런(Rerun) 또는 단순 페이지 갱신 ---")
        self.simulate_ui_render_cycle()
        self.assertEqual(len(self.mock_st_state["messages"]), 2, "리런 후에도 데이터가 유지되어야 합니다.")
        ...
        self.assertEqual(len(final_history), 4, "총 4개의 메시지가 완벽하게 보존되어야 합니다.")
