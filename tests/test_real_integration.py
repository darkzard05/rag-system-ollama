...
async def test_real_app_condition_flow():
    ...
    page.insert_text((50, 150), "The system supports DeepSeek-R1 and other Ollama models.")
    doc.save(pdf_path)
    doc.close()
    ...
    try:
        ...
        full_response = ""
        async for event in qa_chain.astream_events({"input": question}, config=run_config, version="v2"):
        ...
        print("\n검증 결과:")
        print(f" - LangGraph 포함 (검색 성공 여부): {'✅ PASS' if has_langgraph else '❌ FAIL'}")
