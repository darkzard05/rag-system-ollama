...
def legacy_merge(docs):
    if not docs:
        return []
    sorted_docs = sorted(docs, key=lambda d: (d.metadata.get("source", ""), d.metadata.get("page", 0), d.metadata.get("chunk_index", -1)))
    merged = []
    current_doc = Document(page_content=sorted_docs[0].page_content, metadata=sorted_docs[0].metadata.copy())
    ...
    for next_doc in sorted_docs[1:]:
        ...
        if curr_src == next_src and next_idx == curr_idx + 1:
            current_doc.page_content += " " + next_doc.page_content  # <--- Costly operation
            current_doc.metadata["chunk_index"] = next_idx
        else:
            merged.append(current_doc)
            current_doc = Document(page_content=next_doc.page_content, metadata=next_doc.metadata.copy())
    merged.append(current_doc)
    return merged
