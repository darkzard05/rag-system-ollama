...
from src.services.optimization.caching_optimizer import (
    CacheEntry,
    CacheStatistics,
    MemoryCache,
    SemanticCache,
...
class TestResponseCache:
    ...
    async def test_response_set_and_get(self):
        ...
        if result:
            assert result.response == "AI is artificial intelligence" or result == "AI is artificial intelligence"
...
class TestThreadSafety:
    ...
    def test_threading_safety(self):
        ...
        threads = [
            Thread(target=worker, args=(f"key{i}", f"value{i}"))
            for i in range(5)
        ]
