...
class TestRegistry:
    ...
    def test_04_compare_models(self, registry):
        ...
        comparison = registry.compare_models(
            ["Llama 2:7b", "Mistral:7b", "Neural Chat:7b"], metric="latency"
        )
...
class TestComparator:
    ...
    def test_16_compare_by_metric(self, registry):
        ...
        result = comp.compare_by_metric(["Llama 2:7b", "Mistral:7b"], "latency_ms")
    ...
    def test_17_rank_models(self, registry):
        ...
        result = comp.rank_models(
            ["Llama 2:7b", "Mistral:7b"], {"latency_ms": 0.5, "memory_mb": 0.5}
        )
...
class TestSelector:
    ...
    def test_18_select_fastest(self, registry, model_selector):
        ...
        selected = model_selector.select_model(ModelTask.QA, SelectionStrategy.FASTEST)
    ...
    def test_19_select_balanced(self, registry, model_selector):
        ...
        selected = model_selector.select_model(ModelTask.QA, SelectionStrategy.BALANCED)
        assert selected is not None

    def test_20_with_constraint(self, registry, model_selector):
        """제약 조건."""
        constraint = SelectionConstraint(max_memory_mb=2048)
        selected = model_selector.select_model(ModelTask.QA, constraint=constraint)
        assert selected is not None
