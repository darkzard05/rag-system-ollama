...
async def evaluate_hallucination():
    ...
    # 테스트용 제한된 컨텍스트
    mock_docs = [
        Document(page_content="Apple Inc. announced the iPhone 15 in September 2023.", metadata={"page": 1, "source": "tech_news.pdf"}),
        Document(page_content="The iPhone 15 uses a USB-C charging port for the first time in iPhone history.", metadata={"page": 2, "source": "tech_news.pdf"})
        ]
    ...
    for i, case in enumerate(test_cases):
        ...
        judge_text = judge_result.content if hasattr(judge_result, 'content') else str(judge_result)
        
        is_pass = "VERDICT: PASS" in judge_text.upper() or "PASS" == judge_text.strip().split('\n')[0].replace("Verdict:", "").strip().upper()
        results.append({
            "case": case['type'],
            "pass": is_pass,
    ...
    print(f"성공률: {pass_count}/{len(results)} ({pass_count/len(results)*100:.1f}%)")
    print("="*50)
