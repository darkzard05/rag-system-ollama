...
# 기존 로직 임포트 (수정된 버전)
from core.rag_core import _load_pdf_docs, _split_documents, _create_vector_store
...
async def benchmark_indexing():
    ...
    text_embeddings = zip([d.page_content for d in doc_splits], vectors_np)
    _ = FAISS.from_embeddings(text_embeddings, embedder, metadatas=[d.metadata for d in doc_splits])
