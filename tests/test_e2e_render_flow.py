...
async def test_e2e_generation_to_display_flow():
    ...
    # 테스트용 문서 (인용구 테스트용)
    mock_docs = [
        Document(page_content="DeepSeek-R1 is a powerful reasoning model.", metadata={"page": 1, "source": "tech.pdf"}),
        Document(page_content="It supports various local RAG implementations.", metadata={"page": 2, "source": "tech.pdf"})
    ]
    ...
    checks = {
        "구조화된 보고서 형식 (헤더 확인)": "# " in formatted_response and "## " in formatted_response,
        "인용구 툴팁 변환 (HTML 확인)": 'class="tooltip"' in formatted_response,
        "페이지 정보 포함 ([p.1] 확인)": "[p.1]" in formatted_response or "[p.2]" in formatted_response,
