...
from src.common.utils import apply_tooltips_to_response, preprocess_text, clean_query_text
from src.core.rag_core import _load_pdf_docs # Need to mock fitz for this
from src.core.semantic_chunker import EmbeddingBasedSemanticChunker # Need to mock embedder for this

# Mock for Semantic Chunker
class MockEmbedder:
    def embed_documents(self, texts):
        # Return dummy embeddings for testing
        return np.array([[float(ord(c)) for c in text[:10]] + [0]*(10-len(text)) for text in texts])
...
def test_merge_consecutive_chunks_single_doc():
    doc = Document(page_content="Content", metadata={"source": "a.pdf", "page": 1, "chunk_index": 0})
    assert _merge_consecutive_chunks([doc]) == [doc]

def test_merge_consecutive_chunks_no_consecutive():
    docs = [
        Document(page_content="Content 1", metadata={"source": "a.pdf", "page": 1, "chunk_index": 0}),
        Document(page_content="Content 2", metadata={"source": "a.pdf", "page": 3, "chunk_index": 0}),
        Document(page_content="Content 3", metadata={"source": "b.pdf", "page": 1, "chunk_index": 0}),
    ]
    assert _merge_consecutive_chunks(docs) == docs

def test_merge_consecutive_chunks_simple_consecutive():
    docs = [
        Document(page_content="Part 1.", metadata={"source": "a.pdf", "page": 1, "chunk_index": 0}),
        Document(page_content="Part 2.", metadata={"source": "a.pdf", "page": 1, "chunk_index": 1}),
        Document(page_content="Part 3.", metadata={"source": "a.pdf", "page": 1, "chunk_index": 2}),
    ]
    merged_doc = Document(page_content="Part 1. Part 2. Part 3.", metadata={"source": "a.pdf", "page": 1, "chunk_index": 2})
    assert _merge_consecutive_chunks(docs) == [merged_doc]

def test_merge_consecutive_chunks_mixed_consecutive():
    docs = [
        Document(page_content="P1-C1", metadata={"source": "a.pdf", "page": 1, "chunk_index": 0}),
        Document(page_content="P1-C2", metadata={"source": "a.pdf", "page": 1, "chunk_index": 1}),
        Document(page_content="P2-C1", metadata={"source": "a.pdf", "page": 2, "chunk_index": 0}),
        Document(page_content="P2-C2", metadata={"source": "a.pdf", "page": 2, "chunk_index": 1}),
        Document(page_content="B-P1-C1", metadata={"source": "b.pdf", "page": 1, "chunk_index": 0}),
    ]
    expected = [
        Document(page_content="P1-C1 P1-C2", metadata={"source": "a.pdf", "page": 1, "chunk_index": 1}),
        Document(page_content="P2-C1 P2-C2", metadata={"source": "a.pdf", "page": 2, "chunk_index": 1}),
        Document(page_content="B-P1-C1", metadata={"source": "b.pdf", "page": 1, "chunk_index": 0}),
    ]
...
def test_merge_consecutive_chunks_missing_metadata():
    docs = [
        ...
        Document(page_content="Content 3", metadata={"source": "a.pdf", "page": 1, "chunk_index": 0}),
        Document(page_content="Content 4", metadata={"source": "a.pdf", "page": 1, "chunk_index": 1}),
    ]
    expected = [
        Document(page_content="Content 1", metadata={"source": "a.pdf"}),
        Document(page_content="Content 2", metadata={"source": "a.pdf", "page": 1}),
        Document(page_content="Content 3 Content 4", metadata={"source": "a.pdf", "page": 1, "chunk_index": 1}),
    ]
    assert _merge_consecutive_chunks(docs) == expected

def test_merge_consecutive_chunks_large_number_of_docs(benchmark):
    base_doc = Document(page_content="test content", metadata={"source": "large.pdf", "page": 1, "chunk_index": 0})
    docs = [Document(page_content=f"content {i}", metadata={"source": "large.pdf", "page": i // 100 + 1, "chunk_index": i % 100}) for i in range(1000)]
...
def test_apply_tooltips_to_response_citation_for_non_existent_page():
    ...
    assert apply_tooltips_to_response(response, docs) == response # No tooltip for non-existent page
...
@patch('src.core.rag_core.SessionManager')
@patch('src.core.rag_core.monitor')
def test_load_pdf_docs_single_page_valid_text(mock_monitor, mock_session_manager, mock_fitz_open):
    mock_page = MagicMock()
...
@patch('src.core.rag_core.SessionManager')
@patch('src.core.rag_core.monitor')
def test_load_pdf_docs_multiple_pages_mixed_content(mock_monitor, mock_session_manager, mock_fitz_open):
    ...
    mock_page3.get_text.return_value = "Content for page three, which is longer than ten chars."
    ...
    assert docs[0].metadata["page"] == 1
    assert docs[1].page_content == "Content for page three, which is longer than ten chars."
    assert docs[1].metadata["page"] == 3

@patch('src.core.rag_core.fitz.open')
@patch('src.core.rag_core.SessionManager')
@patch('src.core.rag_core.monitor')
def test_load_pdf_docs_get_text_exception(mock_monitor, mock_session_manager, mock_fitz_open):
    mock_page = MagicMock()
...
def test_find_breakpoints_no_breakpoints(semantic_chunker_instance):
    ...
    semantic_chunker_instance.breakpoint_threshold_value = 25.0 # Low percentile means threshold is low, so more breakpoints
...
def test_split_text_single_sentence(semantic_chunker_instance):
    ...
    with patch.object(semantic_chunker_instance, '_split_sentences', return_value=[{"text": "Hello world.", "start": 0, "end": 12}]):
...
def test_split_text_consecutive_short_sentences_merge(semantic_chunker_instance):
    ...
    with patch.object(semantic_chunker_instance, '_split_sentences', return_value=[
...
def test_split_text_short_sentences_no_merge_due_to_min_chunk_size(semantic_chunker_instance):
    # If the current_s_parts is already long, new short sentences won't merge
    with patch.object(semantic_chunker_instance, '_split_sentences', return_value=[
        {"text": "This is a long sentence that already exceeds half min_chunk_size.", "start": 0, "end": 66}, # len 66 > 10/2=5
        {"text": "Ok.", "start": 67, "end": 70}, # len 3
        {"text": "Bye.", "start": 71, "end": 75} # len 4
    ]):
        result = semantic_chunker_instance.split_text("This is a long sentence that already exceeds half min_chunk_size. Ok. Bye.")
        ...
        assert len(result) == 3
        assert result[0]["text"] == "This is a long sentence that already exceeds half min_chunk_size."
...
def test_split_text_with_consecutive_short_sentences_exceeding_max_chunk_size(semantic_chunker_instance):
    # min_chunk_size=10, max_chunk_size=50
    ...
    with patch.object(semantic_chunker_instance, '_split_sentences', return_value=[
        ...
        result = semantic_chunker_instance.split_text("a. b. c. d. e. f. g. h. i. j. k. l. m. n. o. p. q. r. s. t. u. v. w. x. y. z.")
        ...
        assert len(result) > 1
        for chunk in result:
            assert len(chunk["text"]) <= semantic_chunker_instance.max_chunk_size + 1 # +1 for potential space
