...
class TestStreamingResponseHandler(unittest.TestCase):
    ...
    def test_basic_streaming(self):
        ...
        async def _test():
            ...
            metrics = await self.handler.stream_response(
                self._mock_token_generator(["hello", " ", "world"]),
                on_chunk
            )
            
            self.assertGreater(len(chunks_received), 0)
            self.assertGreater(metrics.total_tokens, 0)  # 토큰 개수 확인 (정확히 3개가 아닐 수 있음)
    ...
    def test_streaming_metrics(self):
        ...
        async def _test():
            ...
            metrics = await self.handler.stream_response(
                self._mock_token_generator(["a", "b", "c", "d", "e"]),
                on_chunk
            )
    ...
    def test_streaming_with_completion_callback(self):
        ...
        async def _test():
            ...
            await self.handler.stream_response(
                self._mock_token_generator(["test"]),
                on_chunk,
                on_complete
            )
    ...
    def test_first_token_latency(self):
        ...
        async def _test():
            ...
            metrics = await self.handler.stream_response(
                self._mock_token_generator(["first", "second"]),
                on_chunk
            )
    ...
    def test_avg_chunk_size_calculation(self):
        ...
        async def _test():
            ...
            metrics = await self.handler.stream_response(
                self._mock_token_generator(["a", "bb", "ccc"]),
                on_chunk
            )
    ...
    def test_streaming_with_large_tokens(self):
        ...
        async def _test():
            ...
            metrics = await self.handler.stream_response(
                self._mock_token_generator([large_token, large_token]),
                on_chunk
            )
