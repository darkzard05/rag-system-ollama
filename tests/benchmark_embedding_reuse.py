...
def run_benchmark():
    ...
    test_sentences = [f"이것은 실제와 유사한 성능 측정을 위해 생성된 {i}번째 문장입니다. 문장이 길어질수록 최적화 효과가 극대화됩니다." for i in range(1000)]
    ...
    chunker_legacy = EmbeddingBasedSemanticChunker(embedder=embedder_legacy, min_chunk_size=100, max_chunk_size=500)
    ...
    chunker_opt = EmbeddingBasedSemanticChunker(embedder=embedder_opt, min_chunk_size=100, max_chunk_size=500)
    ...
    if vectors:
        ...
        _ = FAISS.from_embeddings(text_embeddings, embedder_opt, metadatas=[d.metadata for d in split_docs])
    ...
    print(f"호출 감소량: {legacy_calls - opt_calls}회 ({ (legacy_calls - opt_calls)/legacy_calls*100:.0f}% 감소)")
    print("="*50)
