...
async def test_prompt_caching():
    print(f"🚀 [Test] 프롬프트 캐싱(KV Cache) 최적화 검증 시작")
    ...
    async def run_query(query, label):
        ...
        inputs = {
            ...
            "documents": [
                ...
                {"page_content": shared_context[2], "metadata": {"source": "doc.pdf", "page": 3, "chunk_index": 2}},
                {"page_content": shared_context[0], "metadata": {"source": "doc.pdf", "page": 1, "chunk_index": 0}},
                {"page_content": shared_context[1], "metadata": {"source": "doc.pdf", "page": 2, "chunk_index": 1}},
            ]
        }
        ...
        async for event in graph.astream_events(inputs, config=config, version="v2"):
            if event.get("event") == "on_custom_event" and event.get("name") == "response_chunk":
                if ttft == 0:
    ...
    if ttft2 < ttft1:
        ...
        print(f"✅ 결과: 성공! 캐시 활용으로 인해 응답 속도가 대폭 향상되었습니다.")
    else:
        print(f"❌ 결과: 실패 또는 미미함. Ollama 설정이나 자원 상황을 확인해야 합니다.")
    print("="*40)
