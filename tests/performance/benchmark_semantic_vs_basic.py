...
async def run_comparison():
    ...
    print("-" * 60)
    print(f"{ '소요 시간(초)':<20} | {results['Basic']['time']:<18.2f} | {results['Semantic']['time']:<18.2f}")
    print(f"{ '생성된 청크 수':<20} | {results['Basic']['chunk_count']:<18} | {results['Semantic']['chunk_count']:<18}")
    print(f"{ '평균 청크 길이':<20} | {results['Basic']['avg_len']:<18.1f} | {results['Semantic']['avg_len']:<18.1f}")
    print("="*60)

    # 분석 의견
    print("\n📊 분석 결과:")
    if results["Semantic"]["time"] > results["Basic"]["time"]:
        overhead = (results["Semantic"]["time"] - results["Basic"]["time"]) / results["Basic"]["time"] * 100
        print(f"1. 시간 비용: 의미론적 청킹이 약 {overhead:.1f}% 더 소요됩니다.")
    else:
        print("1. 시간 비용: 벡터 재사용 덕분에 의미론적 청킹이 오히려 효율적입니다.")

    print(f"2. 구조적 차이: Basic은 고정 크기로 쪼개지만, Semantic은 {results['Semantic']['chunk_count']}개의 '의미 단위'로 묶었습니다.")
    print("3. 효용성: 현재 구조(Vector Reuse)는 의미론적 청킹 시 발생하는 임베딩 비용을 FAISS 인덱싱 단계에서 100% 회수하므로 매우 효율적입니다.")
