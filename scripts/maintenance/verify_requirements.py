...
def main():
    ...
    for pkg, min_version in security_checks.items():
        if (
            pkg in base_reqs
            and base_reqs[pkg] != min_version
            and not base_reqs[pkg].startswith(min_version)
        ):
            # 간단하게 문자열 비교로 처리
            if pkg == "starlette" and ">=" in base_reqs[pkg]:
                continue
            errors.append(
                f"[Security] {pkg} version should be {min_version} in base.txt"
            )

        if (
            pkg in win_reqs
            and win_reqs[pkg] != min_version
            and not win_reqs[pkg].startswith(min_version)
        ):
            errors.append(
                f"[Security] {pkg} version should be {min_version} in windows.txt"
            )
