...
async def test_real_pdf_structured_output():
    ...
    try:
        ...
        print(
            f"Loaded {len(docs)} pages. Using page {sample_doc.metadata.get('page')} for test."
        )
    except Exception as e:
        print(f"PDF Loading Error: {e}")
        return

    # 3. 통합 Grade/Rewrite 노드 테스트 (실제 문서 내용 활용, 새 API: bind + JSON 파싱)
    ...
    writer = (
        lambda x: print(f"  [STATUS] {x.get('status', '')}")
        if x.get("status")
        else None
    )
    ...
    grade_result = await grade_documents(state, config, writer=writer)
    print(f"Resulting Intent: {grade_result.get('intent')}")

    # 4. 재작성 쿼리 확인
    #    (새 API: 재작성은 grade_documents가 수행하고, rewrite_query는 전달만 함)
    print("\n--- Testing Rewrite (via grade_documents) ---")
    optimized = grade_result.get("search_queries")
    print(f"Optimized Query: {optimized[-1] if optimized else 'N/A'}")
    print(f"Retry Count: {grade_result.get('retry_count')}")

    # rewrite_query 노드 호출 (LLM 없이 재작성 쿼리를 전달만 하는지 확인)
    state_rewrite = {
        "input": "딥식 R1 성능 요약",
        "retry_count": 0,
        "search_queries": [],
    ...
    rewrite_result = await rewrite_query(state_rewrite, config, writer=writer)
    print(f"Rewrite Node Retry Count: {rewrite_result.get('retry_count')}")
