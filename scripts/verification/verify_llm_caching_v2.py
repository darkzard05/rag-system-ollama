...
def test_llm_caching_efficiency_real():
    ...
    # C15: ModelManager._instances LRU 캐시(walk-through OrderedDict)는 삭제됨.
    # 단일 인스턴스 캐싱은 ResourceCoordinator 의 ModelPool 이 소유한다
    # (모델 키: f"llm_{model_name}").
    coordinator = get_resource_manager()
    base_count = sum(1 for k in coordinator.models._pool if model_name in k)
