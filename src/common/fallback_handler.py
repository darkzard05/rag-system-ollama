...
class SimpleFallback(FallbackStrategy):
    ...
    async def execute(self, original_error: Exception, *args, **kwargs) -> Any:
        """기본값 반환"""
        logger.info(f"[Fallback] 단순 폴백 실행: {original_error}")
...
class CachedFallback(FallbackStrategy):
    ...
    async def execute(self, original_error: Exception, *args, **kwargs) -> Any:
        ...
        if cache_key in self.cache:
            logger.info(f"[Fallback] 캐시 폴백: {cache_key}")
...
class DegradedServiceFallback(FallbackStrategy):
    ...
    async def execute(self, original_error: Exception, *args, **kwargs) -> Any:
        """저하된 기능 실행"""
        logger.warning(f"[Fallback] 저하된 서비스 모드: {original_error}")
...
class ChainedFallback(FallbackStrategy):
    ...
    async def execute(self, original_error: Exception, *args, **kwargs) -> Any:
        """연쇄 폴백 실행"""
        logger.info(f"[Fallback] 연쇄 폴백 시작 ({len(self.strategies)}개 전략)")

        for i, strategy in enumerate(self.strategies):
            try:
                logger.debug(f"[Fallback] 전략 {i + 1}/{len(self.strategies)} 시도")
                result = await strategy.execute(original_error, *args, **kwargs)
                logger.info(f"[Fallback] 전략 {i + 1} 성공")
                return result

            except Exception as e:
                logger.warning(f"[Fallback] 전략 {i + 1} 실패: {e}")
                continue
...
class GracefulDegradation:
    ...
    def add_fallback(
        ...
        config = FallbackConfig(
            strategy=strategy, error_types=error_types or [Exception]
        )
    ...
    async def execute(self, func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            last_error = e
            logger.error(
                f"[GracefulDegradation] 원본 함수 실패: {type(e).__name__}: {e}"
            )
        ...
        for i, config in enumerate(self.fallback_chain):
            ...
            if not self._should_use_fallback(last_error, config.error_types):
                logger.debug(f"[GracefulDegradation] 폴백 {i + 1}: 에러 타입 불일치")
                continue
            ...
            try:
                ...
                result = await config.strategy.execute(last_error, *args, **kwargs)

                logger.info(f"[GracefulDegradation] 폴백 {i + 1} 성공")
                return result
        ...
        # 모든 폴백 실패
        logger.error("[GracefulDegradation] 모든 폴백 실패")
        raise last_error
...
class ServiceAvailability:
    ...
    def _update_degradation_level(self) -> None:
        ...
        unavailable_count = sum(
            1 for available in self.available_services.values() if not available
        )
