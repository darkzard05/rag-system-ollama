...
class ModelConfig(BaseModel):
    ...
    temperature: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="생성 온도 (0.0: 결정적, 1.0: 창의적)"
    )
    ...
    top_p: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Nucleus sampling 확률"
    )
    num_ctx: int = Field(
        default=4096,
        ge=512,
...
class EmbeddingConfig(BaseModel):
    ...
    def validate_batch_size(cls, v: int) -> int:
        ...
        if v not in valid_sizes:
            raise ValueError(
                f"배치 크기는 {valid_sizes} 중 하나여야 합니다: {v}"
            )
        return v
...
class ChunkingConfig(BaseModel):
    ...
    chunk_size: int = Field(
        ...
        ge=100,
        le=2000,
        description="청크 크기 (문자)"
    )
    chunk_overlap: int = Field(
        default=100,
        ge=0,
        le=500,
        description="청크 오버랩 (문자)"
    )
    separator: str = Field(
        default="\n\n",
...
class RetrievalConfig(BaseModel):
    ...
    use_bm25: bool = Field(
        default=True,
        description="BM25 검색 사용"
    )
    use_semantic: bool = Field(
        default=True,
        description="시멘틱 검색 사용"
    )
    ...
    def validate_all(self) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if self.model.temperature < 0 or self.model.temperature > 1:
                errors.append(f"모델 온도가 범위를 벗어났습니다: {self.model.temperature}")
            
            if self.embedding.batch_size < 1:
                errors.append(f"임베딩 배치 크기가 너무 작습니다: {self.embedding.batch_size}")
