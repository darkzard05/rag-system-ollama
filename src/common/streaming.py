...
class StreamingResponseHandler:
    ...
    async def stream_graph_events(
        ...
        try:
                    ...
                    if mode == "custom":
                        ...
                        for node_name, node_output in data.items():
                            if node_name == "retrieve":
                                ...
                                if perf:
                                    # 표준 어댑터를 사용하여 실제 토큰 수 반영
                                    standard_stats = adapt_llm_metadata(perf)
                                    self.node_metadata.update(
                                        standard_stats.model_dump()
                                    )
                                    self.metrics.total_tokens = (
                                        standard_stats.token_count
                                    )
                                    input_tokens = standard_stats.input_token_count
            ...
            # 표준 어댑터를 사용하여 최종 성능 지표 확정
            final_stats = adapt_llm_metadata(
                self.node_metadata,
                PerformanceStats(
                    total_time=self.metrics.total_time,
                    ttft=self.metrics.first_token_latency,
            ...
            # TPS 계산을 위해 생성 시간 설정 (전체 시간 - 첫 토큰 지연 시간)
            final_stats.generation_time = max(
                0.1, final_stats.total_time - final_stats.ttft
            )

            final_performance = final_stats.model_dump()
            # 계산된 tps 필드 추가 (computed_field는 model_dump에 포함되지 않을 수 있음)
            final_performance["tps"] = final_stats.tps
