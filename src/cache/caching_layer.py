    ...
    def _evict_if_needed(self):
        ...
        if len(self.entries) >= self.max_size:
            if self.eviction_policy == EvictionPolicy.LRU:
                ...
                # Remove oldest entry
                fifo_key = min(
                    self.entries.keys(), key=lambda k: self.entries[k].timestamp
                )
...
class DiskCache:
    """Disk-based L2 cache with security hardening"""

    def __init__(self, cache_dir: str = "./cache"):
        """
        Initialize disk cache with security manager
        ...
        with self._lock:
            try:
                ...
                metadata = self.security_manager.create_metadata_for_file(
                    str(cache_file), description=f"DiskCache entry: {key[:50]}"
                )
            ...
            try:
                ...
                if not success:
                    self.logger.critical(
                        f"[Security] DiskCache 위협 감지! 캐시를 파기합니다: {error}"
                    )
                    self.delete(key)  # 오염된 파일 삭제
                    self.stats.total_misses += 1
                    return None

                # 검증 통과 후 안전하게 로드
                with open(cache_file, "rb") as f:
                    entry_data = pickle.load(f)  # nosec: 보안 검증 완료 후 로드
    ...
    def delete(self, key: str) -> bool:
        """Delete entry and metadata from disk cache"""
        with self._lock:
            try:
                cache_file = self._get_cache_file(key)
