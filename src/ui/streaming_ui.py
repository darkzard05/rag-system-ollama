        ...
        with chat_container:
            with st.chat_message("assistant", avatar="🤖"):
                ...
                async def on_chunk(chunk: StreamChunk) -> None:
                    ...
                    # 메트릭 기록
                    self.adaptive_controller.record_latency(
    ...
    def _display_final_metrics(
        ...
        try:
            ...
            metric_cols[2].metric(
                "⏰ 처리 시간",
                f"{metrics.total_time:.2f}초"
            )
            
            metric_cols[3].metric(
                "📈 청크 수",
                f"{metrics.chunk_count}개"
            )
            ...
            with metrics_container.expander("📈 상세 메트릭"):
                ...
                detail_cols[0].metric(
                    "첫 토큰 지연",
                    f"{metrics.first_token_latency*1000:.2f}ms"
                )
                
                detail_cols[1].metric(
                    "평균 청크 크기",
                    f"{metrics.avg_chunk_size:.1f} 토큰"
                )
    ...
    def format_response_with_citations(
        ...
        for citation_key, citation_info in citations.items():
            if citation_key in html:
                # 툴팁 추가
                tooltip = (
                    f"📄 {citation_info['source']} "
                    f"(p.{citation_info['page']})"
                )
                link_html = (
...
class StreamingMetricsDisplay:
    ...
    def display_streaming_metrics(metrics) -> None:
        ...
        with col4:
            st.metric(
                "첫 토큰 지연",
                f"{metrics.first_token_latency*1000:.0f}ms"
            )
    ...
    def display_adaptive_metrics(
        ...
        with col2:
            st.metric(
                "버퍼 크기",
                f"{metrics['current_buffer_size']}개"
            )
        
        with col3:
            st.metric(
                "샘플 수",
                f"{metrics['sample_count']:.0f}"
            )
