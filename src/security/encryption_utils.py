...
class DataEncryptor:
    ...
    def decrypt_string(self, encrypted_string: str) -> Optional[str]:
        ...
        try:
            payload = json.loads(
                base64.b64decode(encrypted_string).decode()
            )
        ...
        for key, value in encrypted_data.items():
            if key in sensitive_fields or key in self._field_configs:
                if isinstance(value, str):
                    config = self._field_configs.get(
                        key,
                        {'encrypt': True}
                    )
        ...
        try:
            if sensitive:
                if isinstance(value, dict):
                    encrypted_value = self._encryption_manager.encrypt_sensitive_data(value)
                elif isinstance(value, str):
                    encrypted_value = self._encryption_manager._default_encryptor.encrypt_string(value)
                else:
