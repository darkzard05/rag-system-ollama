...
class RBAC:
    ...
    def check_access(
        ...
        with self._lock:
            ...
            if not user:
                self._log_access(
                    user_id, action, resource, resource_id, False,
            ...
            for role in user.roles:
                ...
                for permission in role.permissions:
                    if (permission.action == action and
                        ...
                        (permission.resource_id is None or
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """통계"""
        with self._lock:
            total_access_allowed = sum(
                1 for log in self._access_logs if log.allowed
            )
            total_access_denied = sum(
                1 for log in self._access_logs if not log.allowed
            )
            ...
            return {
                ...
                'denial_rate': (
                    ...
                    if (total_access_allowed + total_access_denied) > 0 else 0
                )
            }
