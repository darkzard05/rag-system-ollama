        ...
        for attempt in range(self.config.max_attempts):
            try:
                ...
                if attempt == self.config.max_attempts - 1:
                    logger.error(
                        f"[Retry] 최대 재시도 횟수 초과: {func.__name__}"
                    )
...
class TimeoutManager:
    ...
    async def execute_with_timeout(
        ...
        **kwargs
        ...
        try:
            logger.debug(
                f"[Timeout] 타임아웃 {timeout_val}초로 실행: {func.__name__}"
            )
            ...
            logger.error(
                f"[Timeout] 타임아웃 초과 ({timeout_val}초): {func.__name__}"
            )
            raise
...
class AdaptiveTimeout:
    ...
    def __init__(self, initial_timeout: float = 30.0, percentile: float = 0.95):
