...
class ModelCache:
    ...
    def _evict(self) -> bool:
        ...
        if self.config.eviction_policy == ModelEvictionPolicy.LRU:
            ...
            # 가장 먼저 로드된 모델 제거
            fifo_key = min(
                self._load_times.keys(),
                key=lambda k: self._load_times[k]
            )
            lru_key = fifo_key
        else:
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            return {
                ...
                "utilization_percent": int(100 * len(self._cache) / self.config.max_models),
            }
...
class DynamicModelLoader:
    ...
    async def load_model(self, model_key: str, timeout_seconds: float = 30.0) -> Optional[Any]:
        """모델 로드 (캐시 우선).
    ...
    async def _load_model_async(self, model_key: str, timeout_seconds: float) -> Optional[Any]:
        """비동기 모델 로드 (구현 필요)."""
    ...
    async def auto_unload_unused(self, keep_models: int = 1):
        ...
        if len(loaded_keys) > keep_models:
            sorted_keys = sorted(
                loaded_keys,
                key=lambda k: self._load_count.get(k, 0)
            )
