...
# RAG 엔진 캐시 관리자 — pruning/batch-4 에서 cache/engine_cache.py 의 단일
# 소비자(rag_core)로 인라인 이동 (원본 그대로, 로거 이름 "cache.engine_cache"
# 네임스페이스 보존 — 로그 필터/캡처 동작 불변). SessionManager 가 엔진의 단일
# 소스(SSoT)이자 수명주기를 관리한다 (스레드 안전, 세션 생성/삭제 시 정리).
# 문서 해시(file_hash)가 유효성 키이며, 이벤트 루프 id 는 쓰지 않는다 — set/get 이
# 서로 다른 이벤트 루프(업로드 빌드=AsyncWorker 루프, 쿼리 스트림=`asyncio.run` 새
# 루프)에서 실행되므로 루프 id 로 묶으면 항상 무효화된다.
