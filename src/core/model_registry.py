...
class ModelRegistry:
    ...
    def record_metric(
        ...
        with self._lock:
            ...
            metrics.max_latency_ms = (
                max(metrics.latency_history) if metrics.latency_history else 0
            )
            metrics.min_latency_ms = (
                min(metrics.latency_history) if metrics.latency_history else 0
            )
            ...
            if metrics.latency_history:
                ...
                metrics.p99_latency_ms = sorted_latencies[
    ...
    def get_model_stats_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_success_rate = (
                sum(m.success_rate for m in self._metrics.values()) / len(self._metrics)
                if self._metrics
                else 0
            )
            ...
            return {
                ...
                "tasks_supported": len(
                    [t for t in self._task_index if self._task_index[t]]
                ),
            }
