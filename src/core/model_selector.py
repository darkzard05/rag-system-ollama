...
class ModelSelector:
    ...
    def select_model(
        ...
        if selected_model:
            ...
            logger.info(
                f"모델 선택: {task.value} -> {selected_model} ({strategy.value})"
            )
    ...
    def _select_by_strategy(
        ...
        if strategy == SelectionStrategy.FASTEST:
            # 가장 빠른 모델
            comparisons = self.comparator.compare_by_metric(model_keys, "latency_ms")
            return comparisons[0][0] if comparisons else None

        elif strategy == SelectionStrategy.MOST_ACCURATE:
            # 가장 정확한 모델 (품질 점수 높음)
            comparisons = self.comparator.compare_by_metric(model_keys, "quality_score")
            # 내림차순 (높을수록 좋음)
            comparisons.reverse()
            return comparisons[0][0] if comparisons else None
