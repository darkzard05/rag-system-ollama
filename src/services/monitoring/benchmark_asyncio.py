...
class AsyncBenchmark:
    ...
    def compare_results(
        ...
        """결과 비교 및 출력"""
        improvement = (sequential.avg_time - parallel.avg_time) / sequential.avg_time * 100
        ...
        print(f"{'-'*70}")
        print(f"{'평균 시간(ms)':<20} {sequential.avg_time*1000:<20.2f} {parallel.avg_time*1000:<20.2f}")
        print(f"{'최소 시간(ms)':<20} {sequential.min_time*1000:<20.2f} {parallel.min_time*1000:<20.2f}")
        print(f"{'최대 시간(ms)':<20} {sequential.max_time*1000:<20.2f} {parallel.max_time*1000:<20.2f}")
        print(f"{'P95 시간(ms)':<20} {sequential.p95_time*1000:<20.2f} {parallel.p95_time*1000:<20.2f}")
        print(f"{'처리량(calls/s)':<20} {sequential.throughput:<20.2f} {parallel.throughput:<20.2f}")
        print(f"{'-'*70}")
...
async def run_benchmarks():
    ...
    total_seq_time = seq_result_1.avg_time + seq_result_2.avg_time + seq_result_3.avg_time
    total_par_time = par_result_1.avg_time + par_result_2.avg_time + par_result_3.avg_time
