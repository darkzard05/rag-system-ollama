...
class NodeMetrics:
    ...
    def is_healthy(self, cpu_threshold: float = 80.0, memory_threshold: float = 85.0) -> bool:
        """노드 상태 확인."""
        return self.cpu_percent < cpu_threshold and self.memory_percent < memory_threshold
...
class ClusterManager:
    ...
    def get_cluster_status(self) -> Dict:
        ...
        with self._lock:
            ...
            avg_cpu = sum(n.cpu_percent for n in nodes.values()) / len(nodes) if nodes else 0
            avg_memory = sum(n.memory_percent for n in nodes.values()) / len(nodes) if nodes else 0
