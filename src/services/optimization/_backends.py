"""
캐시 백엔드 계층 - Task 13 캐싱 최적화

caching_optimizer 모듈에서 분리된 캐시 백엔드 구현 계층이다. CacheEntry,
CacheBackend, MemoryCache, SemanticCache, DiskCache, ObjectCache,
SyncCacheBridge 그리고 직렬화 포맷 보조 요소(CACHE_FORMAT/CACHE_FORMAT_KEY/
_json_default)가 위치한다. 기존 import 경로
(``services.optimization.caching_optimizer.<symbol>``)는 caching_optimizer
모듈의 재수출로 유지된다.
"""
